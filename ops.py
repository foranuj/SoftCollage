...
def train_imgset(config):
    ...
    G = init_G(config)
    G = [net.to("mps") for net in G]

    logger.info("Initializing optimizer...")
    optimizer = config_optimizer(config, G)
    optimizer.zero_grad()

    logger.info("Initializing loss and metrics...")
    ...
    for n_iter in range(start_n_iter, config.total_iteration):
        ...
        logger.info(
            f"{n_iter}-th iter - mean[-Reward(tree_i)log(P(tree_i;theta)),1<=i<={config.sample_num}]={float(L)}")
...
def main_aic():
    ...
    for imgset_name, imgs in sict.items():
        ...
        print("....")
        print(f"imgset_name={config.imgset_name}")
        print("....")
        ...
        try:
            ...
            print(f"KeyError: No imgset_name={config.output_dir}")
            if os.path.exists(config.output_dir):
                shutil.rmtree(config.output_dir)
                print(f"rm -rf {config.output_dir}")
            continue
