...
def check_gpu(config):
    return config.use_gpu and torch.device("mps")
...
def count_num(func):
    ...
    def wrapper(*args, **kargs):
        ...
        # HARD CODED 14 HERE for an M1 MAC with 14 cores
        nums[thread] = (1 + nums[thread]) % 14

        # print(args[1].size() if torch.is_tensor(args[1]) else type(args[1]),'on cuda:',nums[thread],'by thread',
        # thread)
...
def to_gpu(config, obj, thread=0, cuda_id=0):
    ...
    """
    if config.use_gpu and torch.device("mps"):
        return obj.to(torch.device(f'mps:{cuda_id}'))
...
def sync_cuda_id(src, dst):
    ...
    if torch.is_tensor(src) and torch.is_tensor(dst):
        ...
        raise Exception(
            f"Inputs are supposed to be Torch.Tensor, while type(src)={type(src)} and type(dst){type(dst)}.")
...
def init_G(config):
    ...
    print("G init success!")
    # print(ext, fus, hvc, lrc)
...
def cal_dis(f1, f2, dis_type=False):
    ...
    with torch.no_grad():
        ...
        if dis_type == None or dis_type == 'Euclidean' or dis_type == False:
            ...
            dis = torch.sum((f1 - f2) ** 2 / torch.var(torch.cat([f1.unsqueeze(0), f2.unsqueeze(0)], dim=0), dim=0,
                                                       unbiased=False))  # the square of std Euclidean
        return dis
...
def update_heap(heap, nod_new, name2node, config):
    for node in name2node.values():
        if node.name != nod_new.name:
            heappush(heap, (
                cal_dis(sync_cuda_id(nod_new.feat, node.feat), node.feat, config.NNP).item(),
                (nod_new.name, node.name)))
    return heap
...
def build_tree(config, name2tensor, name2ar, G: list, logger=None):
    ...
    if config.mode == 'train':
        # each of these copies might be moved to a different GPU for parallel computation
        # Torch might have some more recent changes to be able to do this more easily
        exts += [copy.deepcopy(ext) for i in range(num_gpu - 1)]
    ...
    for idx, (name, tensor) in enumerate(name2tensor.items()):
        ...
        feat = ext_(tensor, config.W, config.H) if not config.gpu_balanced else to_gpu(config,
                                                                                       ext_(tensor, config.W, config.H),
                                                                                       thread=1)
    ...
    while len(name2node) > 1:
        ...
        nod_new.prob_H = F.softmax(hvc(nod_new), dim=1)[0][0] if not config.gpu_balanced else to_gpu(config, F.softmax(
            hvc(nod_new), dim=1)[0][0], thread=1)
        nod_new.prob_L = F.softmax(lrc(nod_new), dim=1)[0][0] if not config.gpu_balanced else to_gpu(config, F.softmax(
            lrc(nod_new), dim=1)[0][0], thread=2)
...
def parse_config():
    ...
    parser.add_argument("--config_name", type=str, default="demo")
    args = parser.parse_args()
    config = Pack(json.load(open(os.path.join(BASE_DIR, args.config_name))))
    return config
