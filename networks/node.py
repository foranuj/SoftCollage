...
class SoftNode(GPUModule):
    ...
    def predict(self):
        ...
        """
        if self.children[0] == self.children[1] is None:
    ...
    def cal_loglikelihood(self, hn):
        ...
        """
        if hn.lch == hn.rch is None:
            return torch.tensor(0.)
        ret = torch.log(self.prob_H) if hn.isH else torch.log(1 - self.prob_H)
        ret += self.move_tensor_to_gpu(torch.log(self.prob_L) if hn.isL else torch.log(1 - self.prob_L))
        lch_s, rch_s = self.children if hn.isL else self.children[::-1]
        ret += self.move_tensor_to_gpu(lch_s.cal_loglikelihood(hn.lch))
        ret += self.move_tensor_to_gpu(rch_s.cal_loglikelihood(hn.rch))
        return ret
