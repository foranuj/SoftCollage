...
class SqueezeNet(GPUModule):
    ...
    def __init__(self, pretrained, ar_dim=512, w_dim=256, h_dim=256, info_out_dim=512, bsz=1, *args, **kargs):
        ...
        self.W_DIM = w_dim
        self.H_DIM = h_dim
        self.ar_embedding = nn.Parameter(
            torch.ones(bsz, self.AR_DIM))  # nn.Embedding(1,self.AR_DIM,_weight=torch.ones(1,self.AR_DIM))
        self.w_embedding = nn.Parameter(
            torch.ones(bsz, self.W_DIM))  # nn.Embedding(1,self.W_DIM,_weight=torch.ones(1,self.W_DIM))
        self.h_embedding = nn.Parameter(
            torch.ones(bsz, self.H_DIM))  # nn.Embedding(1,self.H_DIM,_weight=torch.ones(1,self.H_DIM))
